...
long cat(char *out, char *filename, int num, ...)
{
    ...
    if (!file) {
        ...
        char alt[PATH_MAX];
        snprintf(alt, sizeof(alt), "/var/www/onvif/%s", filename);
        file = fopen(alt, "r");
    }
    if (!file) {
...
time_t from_iso_date(const char *date)
{
    ...
    if (sscanf(date, "%04d-%02d-%02dT%02d:%02d:%lfZ", &tt.tm_year, &tt.tm_mon, &tt.tm_mday, &tt.tm_hour, &tt.tm_min, &seconds) != 6) {
        ...
        if (sscanf(date, "%04d-%02d-%02dT%02d:%02dZ", &tt.tm_year, &tt.tm_mon, &tt.tm_mday, &tt.tm_hour, &tt.tm_min) != 5) {
            return 0;
        }
    }
...
/**
 * Construct URI with embedded authentication credentials if provided
 * @param output_buffer Buffer to store the constructed URI
 * @param buffer_size Size of the output buffer
 * @param uri_template URI template with %s placeholder for address
 * @param address IP address to substitute in the template
 * @param username Username for authentication (can be NULL or empty)
